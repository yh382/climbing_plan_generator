...
//
// Window BA — single source of truth is `docs/contracts/pii_keys.json`,
// shared with BE (`lib/sentry_init.py`). The synced copy under
// `src/contracts/` is generated by `scripts/sync-fe-contracts.sh` (run by
// `npm run codegen` / ship-window.sh). Edit the docs/contracts/ source.
import PII_KEYS_LIST from "../contracts/pii_keys.json";
const PII_KEYS = new Set<string>(PII_KEYS_LIST);
