...
// Paper basemaps — generated by scripts/mapstyle/build.mjs from frozen
// upstream snapshots (Track 1 map style pass). Bundled as styleJSON so the
// mute rules live in-repo; satellite stays a stock styleURL.
import outdoorsPaperStyle from './mapstyles/outdoors-paper.json';
import darkPaperStyle from './mapstyles/dark-paper.json';
...
// styleJSON takes a string; serialize the bundled paper styles once at module
// load (not per render).
const OUTDOORS_PAPER_JSON = JSON.stringify(outdoorsPaperStyle);
const DARK_PAPER_JSON = JSON.stringify(darkPaperStyle);
...
export default function MapScreenMapbox({
  ...
  // Paper variants ship as bundled styleJSON; satellite stays a stock URL.
  // `key` is the stable identity feeding the styleLoaded gate below.
  const mapStyle = useMemo(() => {
    ...
    return scheme === 'dark'
      ? ({ url: undefined, json: DARK_PAPER_JSON, key: 'paper-dark' } as const)
      : ({ url: undefined, json: OUTDOORS_PAPER_JSON, key: 'paper-light' } as const);
  }, [styleId, scheme]);
  const [loadedStyleKey, setLoadedStyleKey] = useState<string | null>(null);
  // defensive: gate ShapeSource / overrides behind a style-keyed readiness
  // flag — without this, swapping style can fire updateLayer on a style
  // that has already torn down.
  const styleLoaded = loadedStyleKey === mapStyle.key;
  useEffect(() => {
    setStyleReady(false);
    setLoadedStyleKey(null);
  }, [mapStyle.key]);
        ...
        {MAPBOX_TOKEN ? (
          <MapboxGL.MapView
            ...
            onDidFinishLoadingStyle={() => {
              setLoadedStyleKey(mapStyle.key);
              setStyleReady(true);
            }}
