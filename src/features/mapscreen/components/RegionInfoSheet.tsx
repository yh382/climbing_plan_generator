// src/features/mapscreen/components/RegionInfoSheet.tsx
// Canonical region info sheet — consolidates what used to live in three
// places: the gyms-mode POI card, the crag-mode "详情" sheet, and the
// old /outdoor/region-detail page. Presented from:
//   1. Gyms map → tapping an outdoor region pin (seeded with lightweight
//      data so the header paints instantly while the full fetch runs).
//   2. Crag map / unified map → tapping the region name in the sheet header.
//   3. AreaMenuSheet → tapping the region header card or "Area Info &
...
export interface RegionInfoSheetHandle {
  present: () => void;
  dismiss: () => void;
}

interface RegionInfoSheetProps {
  regionId: string | null;
  context: RegionInfoContext;
  ...
  seedRegion?: RegionInfoSeed | null;
...
const RegionInfoSheet = forwardRef<RegionInfoSheetHandle, RegionInfoSheetProps>(
  (props, ref) => {
    ...
    // Which regionId we've already fetched successfully. Keep the last
    // payload on failure / re-open to avoid blank flashes.
    const loadedIdRef = useRef<string | null>(null);

    const loadRegion = useCallback(
      async (id: string) => {
        if (loadedIdRef.current === id && region) return;
        setLoading(true);
        try {
          ...
          // Keep prior region; caller surface can toast if needed.
        } finally {
          setLoading(false);
        }
      },
    ...
    useEffect(() => {
      ...
      if (loadedIdRef.current !== props.regionId) {
        setRegion(null);
      }
    }, [props.regionId]);
    ...
    const onPresent = useCallback(() => {
      if (props.regionId) {
        void loadRegion(props.regionId);
      }
      props.onPresented?.();
    }, [loadRegion, props]);
    ...
    const header = useMemo(() => {
      const seed = props.seedRegion;
      return {
        id: region?.id ?? seed?.id ?? props.regionId ?? '',
        name: region?.name ?? seed?.name ?? '',
        cover_url: region?.cover_url ?? seed?.cover_url ?? null,
        region: region?.region ?? seed?.region ?? null,
        country: region?.country ?? seed?.country ?? null,
        lat: region?.lat ?? seed?.lat ?? null,
        lng: region?.lng ?? seed?.lng ?? null,
        // BR Track A: top-level children rename crag_count → area_count.
        // Seed prop keeps the legacy name for caller minimum-diff; map
        // through here.
        crag_count: region?.area_count ?? seed?.crag_count ?? 0,
        route_count: region?.route_count ?? seed?.route_count ?? 0,
        boulder_count: region?.boulder_count ?? seed?.boulder_count ?? 0,
      };
    }, [region, props.seedRegion, props.regionId]);
    ...
    const showSkeletonSections = loading && !region;

    const hasDetailContent =
      !!region &&
      (!!region.description ||
        !!region.approach ||
        hasTransport(region.transport) ||
        (region.accommodation?.length ?? 0) > 0 ||
        !!region.safety_notes ||
    ...
    const footer = showFooter ? (
      <PlaceSheetFooter actions={footerActions} unionId="region-info-footer" />
    ) : undefined;

    return (
      <TrueSheet
        ref={sheetRef}
        name="region-info-sheet"
      ...
      >
        ...
        >
          ...
          {/* Seasons + approach meta — compact secondary line below the
              ...
              under the metaRow. */}
          {region?.best_seasons?.length || region?.approach_time_min ? (
            <Text style={styles.smallMeta}>
              {bestSeasonSummary(region?.best_seasons, tr)}
              {region?.approach_time_min
                ? `  ·  ${tr('接近', 'Approach')} ${region.approach_time_min}min${
                    approachDifficultyLabel(region?.approach_difficulty, tr)
                      ? ' ' +
                        approachDifficultyLabel(region?.approach_difficulty, tr)
                      : ''
                  }`
            ...
            <View style={styles.contentCard}>
              {region!.description ? (
                <Section title={tr('描述', 'Description')} colors={colors} first>
                  <Text style={styles.paragraph}>{region!.description}</Text>
                </Section>
              ) : null}
              ...
              {region!.accommodation && region!.accommodation.length > 0 ? (
                <Section title={tr('住宿', 'Accommodation')} colors={colors}>
                  <AccommodationBlock
                    items={region!.accommodation}
                    colors={colors}
                  />
                </Section>
              ) : null}

              {region!.safety_notes || region!.emergency_info ? (
...
RegionInfoSheet.displayName = 'RegionInfoSheet';

export default RegionInfoSheet;
